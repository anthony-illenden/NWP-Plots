...
ncss = dataset.subset()
query = ncss.query()
query.lonlat_box(north=55, south=30, east=-70, west=-115).time(datetime.utcnow() + timedelta(hours=3))
...
#cf = ax.contourf(lon_2d, lat_2d, cape,range=caperange,cmap=plt.cm.Reds,extend='max',alpha=0.7,antialiased=True)
...
cf = ax.contourf(lon_2d, lat_2d, cape, cmap=plt.cm.Reds, range=np.arange(250,6000,250),
                 transform=datacrs, extend='max', antialiased=True)

cbar = plt.colorbar(cf, orientation='horizontal', pad=0, aspect=82, shrink=0.50, label = 'CAPE (J/Kg)')
#cbar.ax.set_xticklabels(['1000','2000','3000','4000','5000'])
...
plt.title('GFS: CAPE and 850/500MB Winds', loc='left')
