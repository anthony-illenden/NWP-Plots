...
## Calculations ##
total_def = mpcalc.total_deformation(u_wind500.to('kt'), v_wind500.to('kt'), dx=13e3*units.meters, dy=13e3*units.meters)

## Smoothing
total_def = ndimage.gaussian_filter(total_def, sigma=2, order=0)
...
cf = ax.contourf(lon_2d, lat_2d, total_def*1e5, range(-80, 81, 1), cmap=plt.cm.bwr_r,
                 transform=datacrs)
plt.colorbar(cf, orientation='horizontal', pad=0, aspect=50, shrink=0.50, label = 'Total Deformation (1x10^-5)')

ax.streamplot(lon_2d, lat_2d, u_wind500, v_wind500, color='black', transform=datacrs, zorder=2, arrowstyle='->',density=1)
plt.title('GFS 500MB: Total Deformation and Streamlines', loc='left')
