...
temp_h1000 = uvar.sel(isobaric=1000*100, method='nearest').isel(time=istep).data
rh_h1000 = ref.sel(isobaric=1000*100, method='nearest').isel(time=istep).data
td_h1000 = mpcalc.dewpoint_from_relative_humidity(temp_h1000 -273.15 * units.degC, rh_h1000)

temp_h850 = uvar.sel(isobaric=h850*100, method='nearest').isel(time=istep).data
rh_h850 = ref.sel(isobaric=h850*100, method='nearest').isel(time=istep).data
td_h850 = mpcalc.dewpoint_from_relative_humidity(temp_h850 -273.15 * units.degC, rh_h850)
